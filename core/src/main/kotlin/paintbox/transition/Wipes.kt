...
open class WipeTransition(
    ...
    override fun render(transitionScreen: TransitionScreen, screenRender: () -> Unit) {
        ...
        if (slopeOnLeft) {
            if (slope > 0f) {
                batch.drawQuad(
                    xOffset, 0f, color, xOffset + slopePart, 0f, color,
                    xOffset + slopePart, camera.viewportHeight, color, xOffset + slopePart, camera.viewportHeight, color
                )
            } else if (slope < 0f) {
                batch.drawQuad(
                    xOffset + slopePart, 0f, color, xOffset + slopePart, 0f, color,
            ...
            if (slope > 0f) {
                batch.drawQuad(
                    xOffset + camera.viewportWidth,
                    0f,
                    color,
                    xOffset + camera.viewportWidth,
                    0f,
                    color,
                    xOffset + slopePart + camera.viewportWidth,
