 ...
 */
fun WaveFunction.getBoomerangValue(
    periodSec: Float,
    ...
    val regularValue = if (sawtooth >= 0.5f) getWaveValue(absPeriod, timeMs, offsetMs) else getInvertedValue(
        absPeriod,
        timeMs,
        offsetMs
    )
 ...
 */
fun WaveFunction.getMappedValue(
    zeroValue: Float,
    oneValue: Float,
    periodSec: Float,
    timeMs: Long = System.currentTimeMillis(),
